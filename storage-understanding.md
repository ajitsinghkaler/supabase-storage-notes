...
To understand the Supabase Storage repository, we need to understand the various components and how they interact with each other. For that, we will break down how the various components in this Supabase Storage repository interact with each other. It has many components, but I will first list and discuss the major ones:

## Core Concepts

* **Storage Backend:** This is the service responsible for managing object storage. It handles two storage backends (S3 and local filesystem), manages metadata in a database, enforces access control (RLS), and provides APIs for clients.
...
* **Metrics:** Prometheus is used for tracking the performance of the application and monitoring all operations.
...
3. **Request Handling and Validation:**
    ...
    * **Schema Validation:** Each route is associated with a JSON schema. The body, params, and querystring of the request are validated using [ajv](https://github.com/ajv-validator/ajv) against the defined schema to ensure that requests are proper.
    * **Plugins:** Several plugins are attached to the request object to perform certain actions.
        ...
        * **`storage`:** Instance of the storage class with all the required backend/database-specific instances of the classes.
        * **`metrics`:** Collects request-level metrics like the number of requests and timing to provide performance data on the application.
...
4. **Storage Layer Interaction:**
    ...
    * **Object Storage:**
        ...
        * **File Backend:** When the storage configuration is local, then the storage operations will interact with a local disk to perform all the file operations.
        * **`Upload` Class:** It handles all the multipart or single-part uploads to the backend; this class also handles the logic when upload limits are defined in the bucket configuration.
...
5. **Image Transformation:**
    ...
    * If the request is for image transformation, the `ImageRenderer` (this is a class that handles the creation of the actual image) is used, and a secure URL is generated for the image proxy to fetch the image from object stores and then transform it as requested.
    * Rate limiting is applied to prevent abuse of the image transformation service when enabled.

6. **TUS Protocol**

    * When the TUS protocol is used to upload files, the request is routed to `TusServer`, which manages the different operations in the TUS protocol and orchestrates all operations.
        * `FileStore` or `S3Store` manages the storage of the upload chunks (when uploading resumable files).
        * `PgLock` manages the concurrency of upload operations. This ensures that two users are not performing operations on a file at the same time.
...
9. **Metrics and Monitoring:**
    ...
    * `pino-logflare`: Used as a custom pino logging strategy to push logs to Logflare.
...
4. **Data Access:** Data is read/written from/to the backend or database.
    * If files are needed to be transferred, then files are downloaded from S3 or the local system.
    * If only metadata is needed, then data is fetched from the database.
5. **Image Transformation:** If transformation is required, then the Imgproxy URL is built and the asset is fetched from it.
6. **Asynchronous Task:** If needed, a task is enqueued for async processing.
7. **Response:** Data is returned to the client.
...
* **Fastify <-> Queue (PgBoss):** Queue jobs are created by Fastify routes and handled by workers in a separate pool, enabling async behavior.
...
* **Internal Components:** The core components use shared utility libraries and classes to perform core operations, like streaming, concurrency control, or error handling.

The Supabase Storage repository is a highly flexible object storage service. It combines a core with a set of features to store and manipulate files. It can be deployed in several ways, either as a single instance or a multi-tenant instance with tenant configurations.
...
* **Observability:** OTel and Prometheus provide visibility into the application's health and performance.
